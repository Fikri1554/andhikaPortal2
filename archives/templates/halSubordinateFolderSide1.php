...
function klikFold(idFoldSelect)
{
	...
	parent.window.frames["halFolderSide2"].frameElement.src = "halSubordinateFolderSide2.php?idFold="+idFoldSelect+"&userIdOwner="+owner;
...
function treeFold(&$html="", $idFoldGet, $foldSub, $CKoneksi, $CFolder, $CFile, $userIdOwnerGet, $CPublic)
{
	$sql = $CKoneksi->mysqlQuery("SELECT * FROM tblfolder WHERE idfoldref='".$idFoldGet."' AND folderowner=".$userIdOwnerGet." AND deletests=0 ORDER BY namefold ASC");
	...
	while ($row = $CKoneksi->mysqlFetch($sql)) 
	{
		...
		treeFold($html, $row['idfold'], $row['foldsub'], $CKoneksi, $CFolder, $CFile, $userIdOwnerGet, $CPublic);
...
if($userIdOwnerGet == "")
{
	$html = "<ul id=\"LinkedList1\" class=\"LinkedList fontMyFolderList\" style=\"list-style:none;margin-left:0;\">&nbsp;</ul>";
}
if($userIdOwnerGet != "")
{
	treeFold($html, "", "", $CKoneksi, $CFolder, $CFile, $userIdOwnerGet, $CPublic);
}
