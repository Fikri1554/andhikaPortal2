...
//Copy Last stock akan mengcopy dari stock 1 bulan lalu.
if($bln == 01) //jika bulan 1, maka bulan seb = 12, dan tahun seb = tahun skrg - 1
{
	$blnSeb = 12;
	$thnSeb = $thn -1;
}
if($bln != 01) //jika bukan bulan 1, maka bulan seb = bulan skrg - 1, dan tahun seb = tahun skrg
{
...
{
	$stockBlnSeb = stockBlnSeb($CKoneksiAtk, $yearMonthGet);
	
	if($stockBlnSeb != 0)
	{
		//echo substr($stockBlnSeb,0,4).substr($stockBlnSeb,4,2);
		$query = $CKoneksiAtk->mysqlQuery("SELECT DISTINCT itemid FROM stock WHERE stockmonth=".substr($stockBlnSeb,4,2)." AND stockyear=".substr($stockBlnSeb,0,4)." AND active='Y' ORDER BY itemid;");
		while($row = $CKoneksiAtk->mysqlFetch($query))
		{
			$itemId = $row['itemid'];
			
			$CKoneksiAtk->mysqlQuery("INSERT INTO stock (itemid, urutanid, stockdate, stockmonth, stockyear, stockname, stocktype, stockmin, stockprice, stockall, lastmonthstock, addusrdt) SELECT stock.itemid, 1, '00', '".$bln."', ".$thn.", stock.stockname, stock.stocktype, stock.stockmin, stock.stockprice, stock.stockall, stock.stockall, '".$CPublic->userWhoAct()."' FROM stock WHERE itemid = ".$itemId." AND stockmonth=".substr($stockBlnSeb,4,2)." AND stockyear=".substr($stockBlnSeb,0,4)." ORDER BY stockid DESC LIMIT 1;");
		}
	}
}

function stockBlnSeb($CKoneksiAtk, $yearMonthGet)
{
	$query1 = $CKoneksiAtk->mysqlQuery("SELECT * FROM stock WHERE CONCAT(stockyear,stockmonth) < ".$yearMonthGet." GROUP BY CONCAT(stockyear,stockmonth) ORDER BY CONCAT(stockyear,stockmonth) DESC LIMIT 1");
	$jml = $CKoneksiAtk->mysqlNRows($query1);
	$row = $CKoneksiAtk->mysqlFetch($query1);
	
	if($jml == 0)	return 0;
	if($jml > 0)	return $row['stockyear'].$row['stockmonth'];	
}
?>
