...
class CEmployee
{
	...
	function menuEmployee($empNo, $cLogin)
	{
		...
		$query = $this->koneksiOdbc->odbcExec($this->koneksiOdbcId, "SELECT empno, nama, tglkeluar, stsresign, YEAR(tglkeluar) AS tahunkeluar FROM dbo.tblMstEmp WHERE (stsresign = 0) AND (kdcmp = 01 OR kdcmp = 02) AND (YEAR(tglkeluar) = '1900') AND (deletests = 0) ORDER BY nama");
	...
	{
		$query = $this->koneksiOdbc->odbcExec($this->koneksiOdbcId, "SELECT  TOP (1) empno, eftivedt, kdcmp, kddiv, kddept, kdjabatan, kdpangkat, addusrdt, updusrdt, deletests, uniquekey FROM dbo.tblEmpGen WHERE (empno = '".$empNo."') ORDER BY eftivedt DESC");
		$row = $this->koneksiOdbc->odbcFetch($query);
	...
	{
		$html = "";
		$html.= "<select class=\"elementMenu\" id=\"divisi\" name=\"divisi\" title=\"Choose Division Name\" ".$dis.">";
