<?php
  /**************************************************
   *  SCRIPTER : Hermawan Haryanto (62-856-1048653) *
...
class myTemplateParser
{
  ...
  var $version;
	...
	* @access private
    */
   function myTemplateParser( $tpl_file, $type )
   {
       $this->version        = '3.0.1';
   ...
   function __prepare()
   {
    ...
    /**
     * myTemplateParser::__prepareTemplate()
     * 
     * @param $tpl_file
     * @param $type
     * @return 
    ...
    function __prepareTemplate( $tpl_file, $type )
    {
        ...
        else
        {
            $this->{$tplvar}["content"] = @file( $tpl_file ) or
                die( $this->__errorAlert('myTemplate Error: Couldn\'t open [ '. $tpl_file .' ]!'));
        }
    ...
    /**
     * myTemplateParser::__parseTemplate()
     * 
     * @param $tplvar
     * @param $blockname
     * @param $initdev
                           ...
                           //check if the includescript file is assigned by the assignInclude function
                            ...
                            if( $include_defined )
                            {
								...
								if( $type == T_BYFILE )
								{
                                    ...
                                    {
                                        $this->__errorAlert( 'myTemplate Error: Couldn\'t include script [ '. $include_file .' ]!' );
										exit();
                                    }
								}
								else
                           ...
                           //do match for 'AS'
                            if (preg_match('/(.+) AS (.+)/', $regs[2], $reuse_regs))
                            {
                               ...
                               //test if original block exist
                                ...
                                {
                                    $this->__errorAlert('myTemplate Error: Can\'t find block \''. $originalbname .'\' to REUSE as \''. $copybname .'\'');
                                }
                            }
                            else
                            {
    ...
    /**
     * myTemplateParser::assignInclude()
     * 
     * @param $iblockname
     * @param $value
     * @param $type
...
class myTemplate extends myTemplateParser
{
  ...
  var $prepared;
	 ...
	 * @access public
     */
    function myTemplate( $tpl_file='', $type= T_BYFILE )
    {
        myTemplateParser::myTemplateParser( $tpl_file, $type );
    ...
    function __deSerializeTPL( $stpl_file, $type )
    {
        if( $type == T_BYFILE )
        {
            $serializedTPL = @file( $stpl_file ) or
                die( $this->__errorAlert('myTemplate Error: Can\'t open [ '. $stpl_file .' ]!'));
        }
        else
        {
  ...
  /**********
    ...
    function printToScreen()
    {
        ...
        {
            $this->__errorAlert('myTemplate Error: Template isn\'t prepared!');
        }
    }

    /**
     * myTemplate::getOutputContent()
     * 
     * @return 
	 * 
	 * @access public
