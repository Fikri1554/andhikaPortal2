...
ORDER BY Acctindo*/
	...
	function detilSplit($idMailInv, $fieldAksi, $urutan, $field)
	{
		$query = $this->koneksi->mysqlQuery("SELECT ".$field." FROM tblsplit WHERE idmailinv='".$idMailInv."' AND fieldaksi='".$fieldAksi."' AND urutan=".$urutan." AND deletests=0;", $this->koneksi->bukaKoneksi());
		$row = $this->koneksi->mysqlFetch($query);
		
		return $row[$field];
	}
	
	function totalAmountSplitDeb($idMailInv, $fieldAksi) // TOTAL AMOUNT YANG DI SPLIT SELAIN URUTAN KE 1
	{
		$query = $this->koneksi->mysqlQuery("SELECT SUM(amount) AS totalamount FROM tblsplit WHERE idmailinv=".$idMailInv." AND fieldaksi='".$fieldAksi."' AND urutan!=1 AND deletests='0';", $this->koneksi->bukaKoneksi());
		$row = $this->koneksi->mysqlFetch($query);
