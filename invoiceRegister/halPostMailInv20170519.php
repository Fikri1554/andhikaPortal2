...
if($aksiPost == "cekSplitAcc")
{
	...
	{
		$CKoneksiInvReg->mysqlQuery("INSERT INTO tblsplittemp (idmailinv, userid, fieldaksi, urutan, booksts, account, amount, description, addusrdt) VALUES ('".$idMailInvPost."', '".$userIdLogin."',  'memodebit', 1, 'DB', '".$debitAccPost."', '".$amountPost."', '".mysql_escape_string($descPost)."', '".$userWhoActNew."')");
	}
	else // JIKA SUDAH ADA DATA DI TBLSPLITTEMP MAKA UPDATE HANYA ACCOUNT YANG NILAINYA DIAMBIL DARI DEBIT ACCOUNT
	{
		$CKoneksiInvReg->mysqlQuery("UPDATE tblsplittemp SET account='".$debitAccPost."' WHERE idmailinv=".$idMailInvPost." AND userid=".$userIdLogin." AND urutan=1 AND fieldaksi='memodebit' LIMIT 1;");
	}
}
...
{
	$idMailInvPost = $_POST['idMailInv'];
	$query = $CKoneksiInvReg->mysqlQuery("SELECT DISTINCT(openpage) AS openpage FROM tblsplittemp WHERE idmailinv='".$idMailInvPost."';",$CKoneksiInvReg->bukaKoneksi());
