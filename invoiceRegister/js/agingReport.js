...
function printAging(jenis)
{
	...
	else
	{
		...
		if(jenis == 'print')
		{
			$('#formPrintAging').attr('action', 'halPrint.php?aksi=printAging&userId='+userId+'&companyName='+companyName+'&fromDate='+fromDate+'&endDate='+endDate+'&account='+account+'&company='+company);
			formPrintAging.submit();
		}
		if(jenis == 'export')
		{
			$("#txtCompanyHid").val($("#company").val());
			$("#txtSdateHid").val($("#fromDate").val());
			$("#txtEdateHid").val($("#endDate").val());
			$("#rbAccountHid").val($('input[name="account"]:checked').val());
			$("#txtUserIdHid").val($("#userId").val());

			formExportAgingPayable.submit();
		}
	}
}