...
function klikBtnRetBatch()
{
	...
	loadIframe('iframeList2', 'templates/halBatchList.php?aksi=header&transNo='+$('#transNo').val());
	
	loadIframe('iframeList3', '');
	loadIframe('iframeList3', 'templates/halBatchList.php?aksi=groupItem&transNo='+$('#transNo').val());
}
...
function klikBtnPageTransno(aksi)
{
	var menuPageTransno = $('#menuPageTransno').val();
	var paramCari = $('#elementCariBatch').val();
	var maxPageTransno = $('#maxPageTransno').html();
	var page = "";
	...
	if(aksi == 'previous')
	{
		...
		{	page = (parseInt(menuPageTransno) - 1);	}
	}
...
function closePopupBatch(barisTransno, idMailInv, transNo)
{
	...
	loadIframe('iframeList', 'templates/halBatchList.php?aksi=displayTransNo&barisTransno='+barisTransno+'&idMailInv='+idMailInv+'&transNo='+transNo+'&paramCari='+paramCari+'&pageTransno='+menuPageTransno); //REFRESH FRAME
	setTimeout(function()
	{
		var tes = window.frames["iframeList"].document.getElementById('tr'+barisTransno).click(); // CLICK ROW YANG SEBELUMNYA AKAN DI PAID
		setTimeout(function()
