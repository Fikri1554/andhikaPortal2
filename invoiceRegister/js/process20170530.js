...
function pilihBtnSaveRet()
{
	...
	for(var i=1; i<=5;i++)
	{
		...
		/*var deducAmount = $("#deducAmount"+i).val().replace(/,/g,"");
		if($.trim( $("#deducAmount"+i).val() ) == "")
		{
			deducAmount = 0;
		}
		
		totalDeduc += parseFloat( deducAmount );*/
		...
		/*var addiAmount = $("#addiAmount"+i).val().replace(/,/g,"");
		if($.trim( $("#addiAmount"+i).val() ) == "")
		{
			addiAmount = 0;
		}
	...
	//alert(totalDeduc);
	//$("#totalDeduc").val( totalDeduc );
	//$("#totalAddi").val( totalAddi );
...
{	
	...
	$('#splitDebAcc').click(function()
	{
		...
		if($('#splitDebAcc').attr('checked') == true) // JIKA AKAN BERI CENTANG ELEMENT SPLIT DEBIT ACCOUNT, DEBIT ACCOUNT TIDAK BOLEH KOSONG DAN KARAKTER HARUS BERJUMLAH 5
		{
			...
			$.post( "../halPostMailInv.php", { aksi:"cekSplitAcc", debitAcc:$('#debitAcc').val(), amount:$('#incomingAmt').val(), desc:$('#description').val(), idMailInv:$('#idMailInv').val() }, function( data )
			{ /*$("#tess").html( data );*/	});
