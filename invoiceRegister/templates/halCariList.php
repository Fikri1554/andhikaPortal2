...
<body>
    ...
    if($aksiGet == "lakukanCari")
    {
        ...
        {
			$paramTeks = " batchno LIKE '%".$teksCariGet."%' AND ";
        }
		if($cariBerdasarkanGet == "company")
        {
            $paramTeks = " companyname LIKE '%".$teksCariGet."%' AND ";
        }
		if($cariBerdasarkanGet == "unit")
        {
            $paramTeks = " unitname LIKE '%".$teksCariGet."%' AND ";
        }
		if($cariBerdasarkanGet == "senVen")
        {
            $paramTeks = " (sendervendor1 LIKE '%".$teksCariGet."%' OR sendervendor2name LIKE '%".$teksCariGet."%') AND ";
        }
        ...
        {
            $paramTeks = "mailinvno LIKE '%".$teksCariGet."%' AND";
        }
		if($cariBerdasarkanGet == "poNumber")
        {
            $paramTeks = "barcode LIKE '%".$teksCariGet."%' AND";	
        }
        if($cariBerdasarkanGet == "remark")
        {
            $paramTeks = "remark LIKE '%".$teksCariGet."%' AND";	
        }
		
		if($startDateGetBatch != "")
		{	$paramStartDt = " batchno >= ".$startDateGetBatch." AND ";	}
		if($endDateGetBatch != "")
		{	$paramEndDt = " batchno <= ".$endDateGetBatch." AND ";	}
		
		$paramCari = $paramTeks.$paramStartDt.$paramEndDt;
		//echo $paramCari;
        ...
        while($row = $CKoneksiInvReg->mysqlFetch($query))
        {
            ...
            if($row['file_upload'] != "")
			{
				$barcodeNya = "<a href=\"./fileUpload/".$row['file_upload']."\" target=\"_blank\" title=\"View File\">".$row['barcode']."</a>";
			}
            ...
            <tr valign="bottom" align="left" bgcolor="<?php echo $rowColor; ?>"
                ...
                <td width="20" height="22" class="tabelBorderBottomJust">&nbsp;<?php echo $imgIcon.$btnBuktiBayar; ?>
                </td>
                <td width="20" class="tabelBorderTopLeftNull" align="center" style="vertical-align: top;">
                    <?php echo $row['urutan']; ?>&nbsp;</td>
                <td width="290" class="tabelBorderTopLeftNull" style="vertical-align: top;">
                    &nbsp;<?php echo $senderVendor; ?></td>
                <td width="65" class="tabelBorderTopLeftNull" align="center" style="vertical-align: top;">
                    <?php echo $barcodeNya; ?></td>
            </tr>
            <?php echo "";
			
			$allIdMailInv.= $row['idmailinv'].",";
        }
        echo $tabel;
    }
    ...
    ?>
            <input type="hidden" id="allIdMailInv" name="allIdMailInv" value="<?php echo $allIdMailInv; ?>"
                style="width:300px;">
        </table>
    </div>
...
<?php
$disableBtnPrint = "parent.disabledBtn('btnCariPrintResult');";
if($allIdMailInv != "")
{
	$disableBtnPrint = "parent.enabledBtn('btnCariPrintResult');";
}

echo $disableBtnPrint."\n\r";
echo "$('#jmlCari', parent.document).html('".$jmlRow."');";
?>
</script>
