...
if($userIdLogin == "00047")//linda
{
	$whereCMP .= " AND unitname LIKE '%SHIP TECHNICAL MANAGEMENT%'";
}
...
function onClickTr(trId, idMailInv, senderVendor, ack, bgColor) {
    ...
    var activeTobePaid = document.getElementById('txtActiveTobePaid_' + idMailInv).value;
    ...
    parent.enabledBtn(
    'btnIncomingEdit'); //BUTTON EDIT AKTIF KETIKA ROW DIKLIK, UNTUK AKSES BISA EDIT APA TIDAK, ADA DI TOMBOL SAVE PADA HALAMAN EDIT, JIKA TIDAK PUNYA AKSES EDIT INCOMING MAKA TIDAK BISA SIMPAN / SAVE JADI HANYA BISA LIHAT DETIL EDITNYA SAJA
    <?php
	echo "\n\r";
	...
	echo "\n\r";
	?>
    if (activeTobePaid == "yes") {
        parent.enabledBtn('btnIncomingTobePaid');
    } else {
        parent.disabledBtn('btnIncomingTobePaid');
    }
}
...
<body onUnload="saveScroll('halIncomingList')">
    ...
    <table id="judul" width="1700" cellpadding="0" cellspacing="0"
        ...
        <tr align="center">
            ...
            <td width="300" rowspan="2" class="">APPROVE / REJECT REMARK</td>
            <td width="70" rowspan="2" class="">BUKTI TRANSFER</td>
            <td width="70" rowspan="2" class="">TRANSNO</td>
        </tr>
        <tr align="center">
...
/*        
		...
		if($row['st_receive'] == "1")
		{
			if($row['another_approve'] == "Y")
			{
				if($row['st_another_approve'] == "1")
				{
					$rowColor = "#86D18C";
				}else{
		...
		if($row['reason_receive'] != "")
		{
			if($row['receivebyname'] != "")
			{
				if($row['another_approve'] == "Y" AND $row['st_another_approve'] == "0")
				{
					$usrRcv = explode("#", $row['receivebyname']);
					$df = explode(" ", $usrRcv[1]);
					$aprRjcRemark .= "Accepted By : ".$usrRcv[0];

					if($row['byname_another_approve'] != "")
					{
						$usrRcv = explode("#", $row['byname_another_approve']);
						$df = explode(" ", $usrRcv[1]);
						$aprRjcRemark .= "<br>Waitting Approval : ".$usrRcv[0];
					}					
				}else{
					$usrRcv = explode("#", $row['receivebyname']);
					$df = explode(" ", $usrRcv[1]);
					$aprRjcRemark .= "Accepted By : <br>- ".$usrRcv[0];

					if($row['byname_another_approve'] != "")
					{
						$usrRcv = explode("#", $row['byname_another_approve']);
						$df = explode(" ", $usrRcv[1]);
						$aprRjcRemark .= ", ".$usrRcv[0];
					}
					$aprRjcRemark .= "<br>".$row['reason_receive'];
				}
			}
		}
		...
		if($row['st_reject'] == "1")
		{
			...
			if($row['rejectbyname'] != "")
			{
				$usrRcv = explode("#", $row['rejectbyname']);
				$df = explode(" ", $usrRcv[1]);
				$txtRejct = "Reject By : - ".$usrRcv[0];
			}
			$aprRjcRemark = $txtRejct."<br><i>(".$row['reason_reject'].")</i>";
		}

		$btnBuktiBayar = "";
		if($row['buktibayar_file'] != "")
		{
			$btnBuktiBayar = "<a href=\"./fileBuktiBayar/".$row['buktibayar_file']."\" target=\"_blank\" title=\"Bukti Transfer\">";
			$btnBuktiBayar .= "<img src=\"../picture/Presentation-blue-32.png\" width=\"20\" title=\"Bukti Transfer\">";
			$btnBuktiBayar .= "</a>";
		}

		$actTobePaid = "no";
		if($compName == "PT. Andhika Lines" AND $row['st_receive'] == "1" AND substr($row['barcode'], 0,1) == "A")
		{
			if($userJenisInvReg == "user")
			{
				if($CInvReg->detilTblUserjns($userIdSession, "btnincoming_tobepaid") == "Y")
				{
					$actTobePaid = "yes";
				}
			}else{
		...
		$transNo = $CPublic->zerofill($row['transno']);
		if($row['transno'] == 0)
		{
			$transNo = "-";
		}
        ...
        <tr valign="top" align="left" bgcolor="<?php echo $rowColor; ?>"
            ...
            <td width="20" height="22" class="tabelBorderBottomJust">
                &nbsp;<?php echo /*$imgMata.$imgGembok; */$imgIcon; ?></td>
            ...
            <td width="130" class="tabelBorderTopLeftNull" align="right">
                &nbsp;<?php echo $currency; ?>&nbsp;<?php echo $CPublic->jikaKosongStrip(number_format((float)$row['amount'], 2, '.', ',')); ?>&nbsp;
            </td>
            ...
            <td width="300" class="tabelBorderTopLeftNull">&nbsp;<?php echo $aprRjcRemark; ?></td>
            <td width="70" class="tabelBorderTopLeftNull" align="center">&nbsp;<?php echo $btnBuktiBayar; ?></td>
            <td width="70" class="tabelBorderTopLeftNull" align="center">&nbsp;<?php echo $transNo; ?></td>
        </tr>
        <input type="hidden" id="txtActiveTobePaid_<?php echo $row['idmailinv']; ?>"
            value="<?php echo $actTobePaid; ?>">
        <?php echo "";
    }
    echo $tabel;
}
