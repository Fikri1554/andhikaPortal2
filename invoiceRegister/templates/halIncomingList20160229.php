...
if($aksiGet == "display")
{
	...
	$tabel = "";
    $i=0;
	$query = $CKoneksiInvReg->mysqlQuery("SELECT * FROM mailinvoice WHERE SUBSTR(batchno, 1, 6)=".$thnBlnGet." AND SUBSTR(batchno, 7, 2)=".$tglGet." AND deletests=0 ".$orderBy.";", $CKoneksiInvReg->bukaKoneksi());
...
if($aksiGet == "cariMailInv")
{
	...
	$tabel = "";
    $i=0;
	$query = $CKoneksiInvReg->mysqlQuery("SELECT *, DATE(SUBSTRING(addusrdt, 7, 8)) AS entrydate FROM mailinvoice WHERE ".$paramCari." deletests=0 ".$orderBy.";", $CKoneksiInvReg->bukaKoneksi());
    while($row = $CKoneksiInvReg->mysqlFetch($query))
    {
        $i++;
        $rowColor = $CPublic->rowColorCustom($i, "#FFFFFF", "#F0F1FF");
