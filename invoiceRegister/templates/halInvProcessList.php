...
{	
	...
	if(saveInvRet == "N")
	{
		...
		parent.document.getElementById("btnUnlockPage").onclick = function onclick()
		{
			$.post( "../halPostMailInv.php", { aksi:"statusOpenPage", idMailInv:idMailInv }, function( data )
			{ // CEK APAKAH HALAMAN RETURN DETAIL / JURNAL ENTRY SEDANG DIBUKA OLEH USER LAIN ATAU TIDAK
				parent.document.getElementById("openPage").value = data;
			...
			setTimeout(function()
			{
				if(parent.document.getElementById("openPage").value == "Y")
				{
					var answer  = confirm('Please make Sure this page isn\'t used!\nAre you sure want to Reset Locked Open Page?');
					if(answer)
					{	
						$.post( "../halPostMailInv.php", { aksi:"unlockPage", idMailInv:idMailInv }, function( data )
						{	
							if(data == "sukses")
							{
								alert( "Page is Succesfully unlocked " );
							}
						}); 
					}
					else
...
?>
        ...
        <tr align="center">
            <td width="60" rowspan="2" height="50" class="">SNO</td>
...
<?php
		$query = $CKoneksiInvReg->mysqlQuery("SELECT * FROM mailinvoice WHERE SUBSTR(batchno, 1, 6)=".$thnBlnGet." AND SUBSTR(batchno, 7, 2)=".$tglGet." AND SUBSTR(barcode, 1, 1)='A' AND deletests=0 AND st_receive = '1' AND IF(another_approve = 'Y',st_another_approve = '1',st_another_approve = '0') ".$whereCMP.$orderBy.";", $CKoneksiInvReg->bukaKoneksi());
...
/*			
			...
			$tabel.=""?>
				...
				style="cursor:pointer;padding-bottom:1px;">
					...
					<td width="16" height="22" class="tabelBorderBottomJust" id="tdIconView<?php echo $i; ?>">&nbsp;<?php echo $imgViewFile; ?></td>
                    ...
                    <td width="75" class="tabelBorderTopLeftNull" align="center"><?php echo $barcode; ?></td>
...
<?php		
		$query = $CKoneksiInvReg->mysqlQuery("SELECT * FROM mailinvoice WHERE SUBSTR(batchno, 1, 6)=".$thnBlnGet." AND SUBSTR(batchno, 7, 2)=".$tglGet." AND SUBSTR(barcode, 1, 1)='A' AND ack=1 AND deletests=0 ".$whereCMP.$orderBy.";", $CKoneksiInvReg->bukaKoneksi());
		while($row = $CKoneksiInvReg->mysqlFetch($query))
		{
			$i++;
			$rowColor = $CPublic->rowColorCustom($i, "#FFFFFF", "#F0F1FF");
...
/*			if($row['tglexp'] == "0000-00-00")
			...
			$tabel.=""?>
				...
				style="cursor:pointer;padding-bottom:1px;">
                    ...
                    <td width="70" class="tabelBorderTopLeftNull" align="center"><?php echo $barcode; ?></td>
