...
function() 
{
	...
	parent.loadScroll('halPaymentList');
...
<body onUnload="parent.saveScroll('halPaymentList')">
...
?>
        ...
        <tr align="center">
            <td width="40" height="50" class="">SNO</td>
...
<?php
		if($aksiGet == "retrieveOutstanding")
		{
			...
			$query = $CKoneksiInvReg->mysqlQuery("SELECT * FROM mailinvoice WHERE SUBSTR(barcode, 1, 1)='A' AND apprpayment=1 AND saveinvret='Y' AND preparepayment='N' AND paid='N' AND ".$paramPrintBy." company='".$companyGet."' AND deletests=0 ".$whereCMP.$orderBy.";", $CKoneksiInvReg->bukaKoneksi());		
		}
		else
		{
			$query = $CKoneksiInvReg->mysqlQuery("SELECT * FROM mailinvoice WHERE SUBSTR(barcode, 1, 1)='A' AND apprpayment=1 AND saveinvret='Y' AND preparepayment='N' AND paid='N' AND deletests=0 ".$whereCMP.$orderBy.";", $CKoneksiInvReg->bukaKoneksi());		
		}
		while($row = $CKoneksiInvReg->mysqlFetch($query))
		{
			...
			$tabel.=""?>        	
				...
				style="cursor:pointer;padding-bottom:1px;">
					...
					<td width="20" height="22" class="tabelBorderBottomJust" align="center"><?php echo $imgViewFile; ?>&nbsp;</td>
					<td width="20" height="22" class="tabelBorderTopLeftNull"><?php echo $row['urutan']; ?>&nbsp;</td>
                    ...
                    <td width="70" class="tabelBorderTopLeftNull">&nbsp;<?php echo $barcode; ?></td>
