<?php
	...
	while($row = $CKoneksiInvReg->mysqlFetch($query))
	{
		...
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['companyname'] = $row['companyname'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['currency'] = $row['currency'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['amount'] = ($row['amount'] + $row['addi']) - $row['deduc'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['addi'] = $row['addi'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['deduc'] = $row['deduc'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['transno'] = $row['transno'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['batchno'] = $tglblnthn;
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['date_send_paymentlist'] = $row['date_send_paymentlist'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['remark_paymentlist'] = $remark;
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['senderVendor'] = $senderVendor;
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['bankcode'] = $row['bankcode'];
		$tempData[strtoupper($row['companyname'])][$bankName][$row['transno']][$noTemp]['settlementTransNo'] = "0";
		$noTemp++;
	}
	...
	while($rows = $CKoneksiVoucher->mysqlFetch($queryVoucher))
	{
		...
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['companyname'] = $rows['companyname'];
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['currency'] = $rows['currency'];
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['amount'] = $rows['amount'];
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['transno'] = "-";
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['batchno'] = $tglblnthn;
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['date_send_paymentlist'] = $rows['date_send_paymentlist'];
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['remark_paymentlist'] = $remarkVoucher;
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['senderVendor'] = $rows['kepada'];
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['bankcode'] = $rows['bankcode'];
		$tempData[strtoupper($rows['companyname'])][$bankName][$noTemp][$noTemp]['settlementTransNo'] = "0";
		$noTemp++;
	}
	...
	while($rowPA = $CKoneksiInvReg->mysqlFetch($queryPA))
	{
		...
		$tempData[strtoupper($rowPA['company_name'])][$bankName][$trnsNo][$noTemp]['idPayment'] = $rowPA['id'];
		$tempData[strtoupper($rowPA['company_name'])][$bankName][$trnsNo][$noTemp]['bankcode'] = $bankCode;
		...
		$tempData[strtoupper($rowPA['company_name'])][$bankName][$trnsNo][$noTemp]['batchno'] = $tglblnthn;//$rowPA['batchno'];
			...
			{				
				...
				foreach ($valZa as $keyZa => $vals)
				{
					...
					if($vals['settlementTransNo'] > 0)
					{
						...
						// $qRmrk = $CKoneksiPaymentVoucher->mysqlQuery("SELECT * FROM payment_split_settlement WHERE id_payment = '".$vals['idPayment']."' AND type_dbcr = 'DB' ;", $CKoneksiPaymentVoucher->bukaKoneksi());

						// while($rowRmrk = $CKoneksiInvReg->mysqlFetch($qRmrk))
						// {
						// 	if($remakExport == "")
						// 	{
						// 		$remakExport = "- ".$rowRmrk['description'];
						// 	}else{
						// 		$remakExport .= "<br>- ".$rowRmrk['description'];
						// 	}
						// }
					}else{
						// $qRmrk = $CKoneksiPaymentVoucher->mysqlQuery("SELECT * FROM payment_split WHERE id_payment = '".$vals['idPayment']."' AND type_dbcr = 'DB' ;", $CKoneksiPaymentVoucher->bukaKoneksi());
