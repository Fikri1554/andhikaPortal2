...
<?php
    $width = "205";
    $totalRowPrepare = totalRowPrepare($CKoneksiInvReg);
...
<?php
function totalRowPrepare($CKoneksiInvReg)
{	
	$query = $CKoneksiInvReg->mysqlQuery("SELECT urutan, mailinvno FROM mailinvoice WHERE SUBSTR(barcode, 1, 1)='A' AND apprpayment=1 AND preparepayment='N' AND paid='N' AND deletests=0");
	$jmlRow = $CKoneksiInvReg->mysqlNRows($query);
