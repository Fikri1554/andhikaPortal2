...
if($aksiGet == "tambahSplit")
{
	...
	$CKoneksiInvReg->mysqlQuery("INSERT INTO tblsplittemp (idmailinv, userid, fieldaksi, urutan, booksts, account, amount, description, addusrdt) VALUES ('".$idMailInvGet."', '".$userIdLogin."', 'memocredit', (SELECT CASE WHEN a.urutan IS NOT NULL THEN (MAX(a.urutan+1)) ELSE '1' END AS urutann FROM tblsplittemp a WHERE a.idmailinv='".$idMailInvGet."' AND a.userid=".$userIdLogin." AND a.fieldaksi='memocredit'), 'CR', '', '', '".$description."', '".$userWhoActNew."')");
}
if($aksiGet == "hapusDesc")
{
	$urutanGet = $_GET['urutan'];
	//echo "<br><br>".$aksiGet." / ".$idMailInvGet." / ".$urutanGet;
	$CKoneksiInvReg->mysqlQuery("DELETE FROM tblsplittemp WHERE  idmailinv=".$idMailInvGet." AND userid=".$userIdLogin." AND fieldaksi='memocredit' AND urutan=".$urutanGet." LIMIT 1;");
}
?>
...
<tr align="center" style="background-color:#F2FAFF;">
    ...
    <td width="129" class="tabelBorderBottomJust">Amount (CR)</td>
...
$tabel = "";
$i=0;
$query = $CKoneksiInvReg->mysqlQuery("SELECT * FROM tblsplittemp WHERE idmailinv='".$idMailInvGet."' AND userid=".$userIdLogin." AND fieldaksi='memocredit' ORDER BY urutan;", $CKoneksiInvReg->bukaKoneksi());
...
while($row = $CKoneksiInvReg->mysqlFetch($query))
{
	...
	{	
		...
		<align=\"center\" valign=\"bottom\">
			<td height=\"23\" width=\"25\" align=\"center\">".$i."</td>
			<td width=\"62\" align=\"right\"><input type=\"text\" id=\"account".$urutan."\" name=\"account".$urutan."\" class=\"elementInput3\" maxlength=\"5\" style=\"width:99%;\" value=\"".$row['account']."\" onBlur=\"simpanSplitCre(this.value, '".$urutan."', 'account');\" disabled></td>
			<td width=\"129\" align=\"right\"><input type=\"text\" id=\"amount".$urutan."\" name=\"amount".$urutan."\" class=\"elementInput3\" style=\"width:99%;text-align:right;\" value=\"".number_format((float)$row['amount'], 2, '.', ',')."\" onKeyUp=\"simpanAmountSplitCre(this.value, '".$urutan."');return false;\" onFocus=\"hanyaAngka".$urutan."();return false;\"></td>
			<td width=\"52\" align=\"right\"><input type=\"text\" id=\"vslCode".$urutan."\" name=\"vslCode".$urutan."\" class=\"elementInput3\" maxlength=\"3\" style=\"width:98%;text-align:right;\" value=\"".$row['vslcode']."\" onBlur=\"simpanSplitCre(this.value, '".$urutan."', 'vslCode');\" disabled></td>
			<td width=\"\"><input type=\"text\" id=\"description".$urutan."\" name=\"description".$urutan."\" class=\"elementInput3\" maxlength=\"70\" style=\"width:99%;\" value=\"".$row['description']."\" onBlur=\"simpanSplitCre(this.value, '".$urutan."', 'description');\"></td>
		</tr>";
	}
	else
	{
		...
		<tr align=\"center\" valign=\"bottom\">
			...
			<td align=\"right\"><input type=\"text\" id=\"account".$urutan."\" name=\"account".$urutan."\" class=\"elementInput3\" maxlength=\"5\" style=\"width:99%;\" value=\"".$row['account']."\" onBlur=\"simpanSplitCre(this.value, '".$urutan."', 'account');\"></td>
			<td align=\"right\"><input type=\"text\" id=\"amount".$urutan."\" name=\"amount".$urutan."\" class=\"elementInput3\" style=\"width:99%;text-align:right;\" value=\"".number_format((float)$row['amount'], 2, '.', ',')."\" onKeyUp=\"simpanAmountSplitCre(this.value, '".$urutan."');return false;\" onFocus=\"hanyaAngka".$urutan."();return false;\"></td>
			<td width=\"52\" align=\"right\"><input type=\"text\" id=\"vslCode".$urutan."\" name=\"vslCode".$urutan."\" class=\"elementInput3\" maxlength=\"3\" style=\"width:98%;text-align:right;\" value=\"".$row['vslcode']."\" onBlur=\"simpanSplitCre(this.value, '".$urutan."', 'vslCode');\"></td>
			<td><input type=\"text\" id=\"description".$urutan."\" name=\"description".$urutan."\" class=\"elementInput3\" maxlength=\"70\" style=\"width:99%\" value=\"".$row['description']."\" onBlur=\"simpanSplitCre(this.value, '".$urutan."', 'description');\"></td>
		</tr>";
	}
}
...
?>
	...
	setTimeout(function()
	{
		parent.totalBalanceSplitCredit();
	},100);
}
...
function klikBtnDelete(urutan)
{
	...
	if(answer)
	{	
		parent.loadIframe("iframeSplitCreList", "");
		parent.loadIframe("iframeSplitCreList", "../templates/halSplitCreList.php?aksi=hapusDesc&urutan="+urutan+"&idMailInv=<?php echo $idMailInvGet; ?>");
	}
	else
	{	return false;	}
}

function klikBtnCreditDelete(urutan)
{
	var answer = confirm('Are you sure want to delete?');
	if(answer)
	{	
...
// 		{ 
// 			aksi:"simpanSplitDeb", idMailInv:"<?php echo $idMailInvGet; ?>", nilaiElement:nilaiElement, urutan:urutan, idElement:idElement 
// 		}, function( data )
// 	{	});
// }

function simpanSplitCre(nilaiElement, urutan, idElement)
{
	$.post( "../halPostMailInv.php", 
		{ 
			aksi:"simpanSplitCre", idMailInv:"<?php echo $idMailInvGet; ?>", nilaiElement:nilaiElement, urutan:urutan, idElement:idElement 
		}, function( data )
	{	});
}

function simpanAmountSplitCre(nilaiElement, urutan)
{
	var incomingAmt = $("#incomingAmt", parent.document).val();
	$.post( "../halPostMailInv.php", { aksi:"simpanSplitCre", idMailInv:"<?php echo $idMailInvGet; ?>", nilaiElement:nilaiElement, urutan:urutan, idElement:"amount" }, function( data )
	{});
...
function updateTotalSplitDeb(totalRow)
{
	...
	for(var i=0; i<=(totalRow-1);i++)
	{
		...
		totalSplitCre += parseFloat( amount );
	}
	$("#totalSplitCre", parent.document).val( "" );
	$("#totalSplitCre", parent.document).val( totalSplitCre );
}

function updateTotalDebAmt()
{
	var totalDebAmt = 0;
	var totalSplitDeb = parseFloat($("#totalSplitCre", parent.document).val());
	var totalAddi = parseFloat($("#totalAddi", parent.document).val());
	
	totalDebAmt = (totalSplitDeb + totalAddi);

	$("#spanTotalCreAmt", parent.document).html( "" );
	$("#spanTotalCreAmt", parent.document).html( formatNumber(totalDebAmt) );
	$("#totalCreAmt", parent.document).val( "" );
	$("#totalCreAmt", parent.document).val( totalDebAmt );
}
</script>