...
if($aksiGet == "addOtherApp")
{
	...
	if($appName == "invoiceRegister")
	{
		...
		$usrName = $CLogin->detilLogin($userIdHAccess, "username");
		$CKoneksi->mysqlQuery("DELETE FROM invoiceregister.tbluserjenis WHERE userid = '".$userIdHAccess."' ");
		$CKoneksi->mysqlQuery("INSERT INTO invoiceregister.tbluserjenis(userid,username,userjenis,btnincoming,btnincoming_edit,btninvprocess,btninvprocess_cekack,btninvprocess_detailack,btninvprocess_detailret,btnpayment,btnpayment_outstanding,btnpayment_detailoutstanding,btnpayment_preppay,btnpayment_addgroupprep,btnpayment_assignprep,btnpayment_resetgroupprep,btnpayment_bybatch,btnpayment_retbatch,btnpayment_transacc,btnpayment_printvoucher,btnpayment_cancelled,btnoutgoing,btnprint,btnprintmailinv,btnprintaging)VALUES('".$userIdHAccess."','".$usrName."','user','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y','Y')");
	}
	if($appName == "budget")
	{
		$appFullNm = "Budget";
	...
	if($appName == "voucher")
	{
		$appFullNm = "VOUCHER";
		$usrName = $CLogin->detilLogin($userIdHAccess, "username");
		$CKoneksi->mysqlQuery("DELETE FROM voucher.tbluserjenis WHERE userid = '".$userIdHAccess."' ");
		$CKoneksi->mysqlQuery("INSERT INTO voucher.tbluserjenis(userid,username,userjenis,diffYear,usercmp)VALUES('".$userIdHAccess."','".$usrName."','user','Y','AND')");
	}
	if($appName == "surveystatus")
	{
		$appFullNm = "SURVEY STATUS";
	}
	if($appName == "jobRole")
	{
		$appFullNm = "JOB ROLE ACTIVITY";
	}
	if($appName == "vesselReport")
	{
		$appFullNm = "VESSEL REPORT";
	}
	if($appName == "shipManagement")
	{
		$appFullNm = "SHIP MANAGEMENT";
	}
	if($appName == "shipOperation")
	{
		$appFullNm = "SHIP OPERATION";
	}
	if($appName == "paymentAdvance")
	{
		$appFullNm = "PAYMENT & ADVANCE";
		$usrName = $CLogin->detilLogin($userIdHAccess, "username");
		$CKoneksi->mysqlQuery("INSERT INTO paymentadvance.tbluserjenis(userid,username,userjenis)VALUES('".$userIdHAccess."','".$usrName."','user')");
	}
