		...
		if(isset($jenisReport))
		{
			...
			$isiMessage.= "<tr>
							<td>
							   ...
							   ".strip_tags($this->isiEmailCC($jenisReport, $rowGuid),'<tr><td><br><span>')."
							   </table>
							</td>
						</tr>";
			//strip_tags untuk menghapus tag html kecuali yang di sebutkan
			// $isiMessage.= "<tr>
							// <td>
							   // <table width=\"100%\" border=0 cellpadding=\"2\" cellspacing=\"2\">
	...
	function isiEmailCC($jenisReport, $rowGuid)
	{
		...
		if($jenisReport == "port")
		{
			...
			for($i = 0; $i < count($arrayTd); $i++)
			{
				...
				$isiSpan = $arrayTd[$i][2];
				//$html.= "<tr><td>".$judul." : <a>".$isi."</a> <span>".$isiSpan."</span></td></tr>";
				$html.= "".$judul." : <a>".$isi."</a> <span>".$isiSpan."</span><br><br>";
			}
			$html.= "</td></tr>";
		}
		if($jenisReport == "maintenance")
		{
			...
			$html.= "<tr><td>LAST RECEIVE IN HO : ".date("d-m-Y H:i:s")." WIB</td></tr>";
