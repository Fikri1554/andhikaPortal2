...
if($aksiGet == "simpanNewEmailEop")
{
	//$messageEncPost = $_POST['messageEnc']; // MESSAGE HASIL ENCRYPT DARI KAPAL
	//$jenisReportPost = $_POST['jenisReport']; // JENIS REPORT APAKAH EOP, COP, NOON REPORT ATAU MORNING REPORT
	//$jenisReportGet = $_Get['jenisReport']; // JENIS REPORT APAKAH EOP, COP, NOON REPORT ATAU MORNING REPORT
	$idtrmencGet = $_GET['idtrmenc'];
	
	$queryEnc = $CKoneksiVslRep->mysqlQuery( "SELECT isienc FROM tblterimaenc WHERE idtrmenc='".$idtrmencGet."' AND reporttype='eop' AND usee='N' LIMIT 1;" );
	$rowEnc = $CKoneksiVslRep->mysqlFetch($queryEnc);
	$messageEnc = $rowEnc['isienc'];
	

	$decTeks = $CVslRep->decrypted("andhikalain", $messageEnc); // DECRYPT MESSAGE DENGAN KATA SANDI "4NDH1K4"
	...
	$CKoneksiVslRep->mysqlQuery( "INSERT INTO tbleop_del SELECT * FROM tbleop WHERE deletests=1;", $CKoneksiVslRep->bukaKoneksi() ); // DATA YANG LAMA SIMPAN KE TABEL tbleop_del
	$CKoneksiVslRep->mysqlQuery( "DELETE FROM tbleop WHERE deletests=1;", $CKoneksiVslRep->bukaKoneksi() );	// HAPUS DATA DI tbleop YANG TIDAK DIPERLUKAN
	
	$query = $CKoneksiVslRep->mysqlQuery( "SELECT refno, namakapal, nameport, lastreceive FROM tbleop WHERE deletests=0 ORDER BY lastreceive DESC LIMIT 1;", $CKoneksiVslRep->bukaKoneksi() ); // QUERY DATA EOP YANG BARU SAJA DISIMPAN DARI VESSEL
	$row = $CKoneksiVslRep->mysqlFetch($query);
	
	//$timeReceive = $CPublic->convTglBlnSingkat($row['lastreceive']);
	$isiPesan = "New EOP Data From ".$row['namakapal'].", Ref No : ".$row['refno'].", Port : ".$row['nameport'];
	...
	{
		$CKoneksiVslRep->mysqlQuery("INSERT INTO tblpesan (userid, reporttype, isipesan, idtrmenc, timerec) VALUES ('".$rowUser['userid']."', 'eop', '".$isiPesan."', '".$idtrmencGet."', NOW());", $CKoneksiVslRep->bukaKoneksi() );
	}
	
	$queryEnc = $CKoneksiVslRep->mysqlQuery( "UPDATE tblterimaenc SET usee='Y' WHERE idtrmenc='".$idtrmencGet."';" );
	
	//echo "New EOP data from ".$row['namakapal']." (".$timeReceive.") \n Ref No : ".$row['refno']." \n Port : ".$row['nameport']." \n Please Refresh Page...";
	echo $isiPesan;
}

if($aksiGet == "simpanNewEmailCop")
{
	$idtrmencGet = $_GET['idtrmenc'];
	
	$queryEnc = $CKoneksiVslRep->mysqlQuery( "SELECT isienc FROM tblterimaenc WHERE idtrmenc='".$idtrmencGet."' AND reporttype='cop' AND usee='N' LIMIT 1;" );
	$rowEnc = $CKoneksiVslRep->mysqlFetch($queryEnc);
	$messageEnc = $rowEnc['isienc'];
	...
	$CKoneksiVslRep->mysqlQuery( "INSERT INTO tblcop_del SELECT * FROM tblcop WHERE deletests=1;", $CKoneksiVslRep->bukaKoneksi() ); // DATA YANG LAMA SIMPAN KE TABEL tbleop_del
	$CKoneksiVslRep->mysqlQuery( "DELETE FROM tblcop WHERE deletests=1;", $CKoneksiVslRep->bukaKoneksi() );	// HAPUS DATA DI tbleop YANG TIDAK DIPERLUKAN
	
	$query = $CKoneksiVslRep->mysqlQuery( "SELECT refno, namakapal, lastport, nextport, lastreceive FROM tblcop WHERE deletests=0 ORDER BY lastreceive DESC LIMIT 1;", $CKoneksiVslRep->bukaKoneksi() ); // QUERY DATA EOP YANG BARU SAJA DISIMPAN DARI VESSEL
	$row = $CKoneksiVslRep->mysqlFetch($query);

	$isiPesan = "New COP Data From ".$row['namakapal'].", Ref No : ".$row['refno'].", Last Port : ".$row['lastport'].", Next Port : ".$row['nextport'];
	
	$queryUser = $CKoneksiVslRep->mysqlQuery("SELECT userid, username FROM tbluserjenis WHERE terimapesan='Y';", $CKoneksiVslRep->bukaKoneksi() );
	while($rowUser = $CKoneksiVslRep->mysqlFetch($queryUser))
	{
		$CKoneksiVslRep->mysqlQuery("INSERT INTO tblpesan (userid, reporttype, isipesan, idtrmenc, timerec) VALUES ('".$rowUser['userid']."', 'cop', '".$isiPesan."', '".$idtrmencGet."', NOW());", $CKoneksiVslRep->bukaKoneksi() );
	}
	$queryEnc = $CKoneksiVslRep->mysqlQuery( "UPDATE tblterimaenc SET usee='Y' WHERE idtrmenc='".$idtrmencGet."';" );
...
if($aksiPost == "checkBtnNotifEop")
{
	...
	echo "<button type=\"button\" class=\"btnStandar\" id=\"btnNotif\" onclick=\"klikBtnNotif('eop');return false;\" title=\"MESSAGE FROM SYSTEM\">
            <table cellpadding=\"0\" cellspacing=\"0\" width=\"".$width."\" height=\"24\"><tr><td align=\"center\"><img src=\"picture/".$icon."\" style=\"vertical-align:middle;\"/><span style=\"vertical-align:middle;margin-left:4px;\">MESSAGE (".$jmlRow.")</span></td></tr></table>
        </button>";
}

if($aksiPost == "checkBtnNotifCop")
{
	...
	echo "<button type=\"button\" class=\"btnStandar\" id=\"btnNotif\" onclick=\"klikBtnNotif('cop');return false;\" title=\"MESSAGE FROM SYSTEM\">
            <table cellpadding=\"0\" cellspacing=\"0\" width=\"".$width."\" height=\"24\"><tr><td align=\"center\"><img src=\"picture/".$icon."\" style=\"vertical-align:middle;\"/><span style=\"vertical-align:middle;margin-left:4px;\">MESSAGE (".$jmlRow.")</span></td></tr></table>
        </button>";
}
