...
function pilihBtnSave()
{
	...
	var yearProcess = $('#yearProcess').val();
	var paramCari = $('#elementCariVoucher').val();
	var saveDiffYear = $('#saveDiffYear').val();
	...
	if( $.trim( yearProcess ) != ambilTahun(datePaid) )
	{
		...
		if(saveDiffYear == "Y")
		{
			var answer  = confirm("Year is not "+yearProcess+", Are you sure want to Save?");
			if(!answer)
			{	return false;	}
		}
	}
	...
	if(invNo == '')
	{
		pesanError('INVOICE NO EMPTY..!!', 'invNo');
		return false;
	}
...
function klikRowVoucher(batchno, transferAcct, voucherType, paidTo, company, payType, bankCode, voucher, reference, chequeNumber, invNo, jobNo, datePaid, curr, amount, additional,txtBarcodeHid,dueDate)
{
	kosongkanElementHid();
	...
	$('#txtBarcodeHid').val(txtBarcodeHid);
	$('#txtDueDateHid').val(dueDate);
...
function allElemReadOnlyFalse()
{
	...
	$('#txtBarcode').prop("readonly", false);
}
...
function klikBtnRetrieve()
{
	...
	$('#txtBarcode').val( $('#txtBarcodeHid').val() );
	$('#txtDueDate').val( $('#txtDueDateHid').val() );
...
function pilihBtnAddDescAll()
{
	var ttlDesc = prompt("Masukan Jumlah Row Deskripsinya..!!","1");
	if(ttlDesc != null)
	{
		if(ttlDesc != "")
		{
...
function pilihBtnSaveDesc()
{
	...
	for(var i=0; i<=(pjgSplitAllIdDesc-2);i++)
	{
		...
		allVoyageNo += '&voyageNo'+splitAllIdDesc[i]+'='+window.frames['iframeDescList'].$('#slcVoyage'+splitAllIdDesc[i]).val();
	...
	{	
		...
		loadIframe('iframeDescList', '');
		loadIframe('iframeDescList', 'templates/halDescList.php?aksi=saveDesc&idVoucher='+idVoucher+'&batchno='+batchno+'&bookStsVoucher='+bookStsVoucher+'&allIdDesc='+allIdDesc+allKetDesc+allDescBookSts+allDescAmount+allPerkiraan+allSubAcc+allDescDiv+allBooksts+allVesCode+allVoyageNo);
	}
	else
	{	return false;	}
}
...
function pilihBtnTranstoAcct()
{
	...
	{	
		...
		loadIframe('iframeVoucherList', '');
		loadIframe('iframeVoucherList', 'templates/halVoucherList.php?aksi=transToAcct&idVoucher='+idVoucher+'&batchno='+batchno+'&pageBatchno='+page+'&yearProcess='+yearProcess+'&company='+company+'&datePaid='+datePaid+'&txtBarcode='+txtBarcode); //REFRESH FRAME
	}
	else
	{	
		doneWait();
...
function cekStatusApprove(idVoucher)
{
	$.post("halPostVoucher.php",
	{ aksi : "cekStatusApprove",idVoucher : idVoucher },
		function(data)
		{
			if(data['statusNya'] == "Y")
			{
				// disabledBtn('btnApprvVoucher');
				$('#btnApprvVoucher').attr('onclick',"unApproveDataVoucher('"+idVoucher+"');");
				$("#idTrBtnApprove").text('Un Approve');
				enabledBtn('btnApprvVoucher');
			}else{
				$("#btnApprvVoucher").attr('onclick',"approveDataVoucher('"+idVoucher+"');");
				$("#idTrBtnApprove").text('Approve');
				enabledBtn('btnApprvVoucher');
			}

			if(data['displayBtn'] == "N")
			{
				disabledBtn('btnApprvVoucher');
			}
		},
