...
function pilihBtnSaveDesc()
{
	...
	{	
		...
		loadIframe('iframeDescList', '');
		loadIframe('iframeDescList', '../templates/halPopupDescList.php?aksi=saveDesc&idVoucher='+idVoucher+'&batchno='+batchno+'&bookStsVoucher='+bookStsVoucher+'&allIdDesc='+allIdDesc+allKetDesc+allDescBookSts+allDescAmount+allPerkiraan+allSubAcc+allDescDiv+allBooksts+allVesCode+allVoyageNo);
	}
	else
	{	return false;	}
...
function klikBtnCari(yearProcess)
{
	...
	$('#idErrorMsg').css('visibility','hidden'); 
	if((cariBerdasarkan == "paidName" || cariBerdasarkan == "company" || cariBerdasarkan == "bank" || cariBerdasarkan == "voucherNo" || cariBerdasarkan == "invNo" || cariBerdasarkan == "currency" || cariBerdasarkan == "totalAmount") && $.trim( teksCari ) == '')
	{
		pesanError("Please types in your search", "teksCari");
		return false;
	}
	if(cariBerdasarkan == "totalAmount")
	{
		var pisahAmount = $.trim( teksCari ).split("-"); 
		if( $.trim( teksCari ).indexOf("-") !== -1 ) // JIKA MENGANDUNG KARAKTER '-'
		{
			if(isNaN(pisahAmount[0]) || isNaN(pisahAmount[1])) // JIKA SETELAH DIPISAH BERDASARKAN '-' DAN MENGANDUNG SELAIN NUMERIC
			{
				pesanError("Please types your search in number", "teksCari");
				return false;
			}
		}
		else // JIKA TIDAK MENGANDUNG KARAKTER '-'
		{
			if(isNaN($.trim( teksCari )))
			{
				pesanError("Please types your search in number", "teksCari");
				return false;
			}
...
function klikBtnResult(yearProcess)
{
	...
	//alert(yearProcess+' / '+idVoucher+' / '+page);
	//return false
	parent.tb_remove(false);
...
function closePopupaaa(idVoucher)
{
	parent.tb_remove(false);
