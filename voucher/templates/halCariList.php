...
<body>
    ...
    <div style="height:298px;overflow-y: scroll;">
        <table id="judul" width="445" cellpadding="0" cellspacing="0"
            ...
            <tr align="center">
                <td width="30" height="30" class="tabelBorderRightJust">NO</td>
        ...
        <table width="445" cellpadding="0" cellspacing="0" style="font:0.7em sans-serif;color:#333;margin-top:30px;">
            <?php
	if($aksiGet == "lakukanCari")
    {
		...
		if($cariBerdasarkanGet == "currency")
        {
            $paramCari = "currency='".$teksCariGet."' AND";
        }
		if($cariBerdasarkanGet == "totalAmount")
        {
			if (strpos($teksCariGet, "-") !== false) // JIKA MENGANDUNG KARAKTER '-'
			{
				$pisahAmount = explode("-", $teksCariGet);
				$paramCari = "(amount >= ".number_format((float)$pisahAmount[0], 2, '.', '')." AND amount <= ".number_format((float)$pisahAmount[1], 2, '.', '').") AND";
			}
			else // JIKA TIDAK MENGANDUNG KARAKTER '-'
			{
				$paramCari = "amount = ".number_format((float)$teksCariGet, 2, '.', '')." AND";
			}
        }
		//echo "<br>".$paramCari;
        ...
        while($row = $CKoneksiVoucher->mysqlFetch($query))
        {
			...
			$menuPageBatchno = cariPage($CKoneksiVoucher, $yearProcessGet, $limitVoucher, $row['idvoucher'] );
            ...
            <tr valign="bottom" align="left" bgcolor="<?php echo $rowColor; ?>"
                ...
                title="<?php echo $titleRow; ?>">
                <td width="30" height="22" class="tabelBorderTopLeftNull" align="center"><?php echo $i; ?></td>
                <td width="65" class="tabelBorderTopLeftNull" align="center">
                    <?php echo $imgIcon."&nbsp;".$CPublic->zerofill($row['batchno'], 6); ?></td>
                <td width="235" class="tabelBorderTopLeftNull" style="padding-left:2px;">
                    <?php echo  $row['kepada']; ?>&nbsp;</td>
                <td width="" class="tabelBorderTopLeftNull" style="padding-left:2px;">
...
<?php
function cariPage($CKoneksiVoucher, $yearProcess, $limitVoucher, $idVoucherParam)
{
	$totalPage = totalPage($CKoneksiVoucher, $yearProcess); // TOTAL KESELURUHAN DATA Batchno
	...
	for($a=1;$a<=$maxPage;$a++)
	{
		...
		$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher FROM tblvoucher WHERE deletests=0 AND YEAR(datepaid)='".$yearProcess."' GROUP BY batchno ORDER BY 0+batchno DESC LIMIT ".$offset.",".$limitPage.";", $CKoneksiVoucher->bukaKoneksi());
		while($row = $CKoneksiVoucher->mysqlFetch($query))
		{
			$allIdVoucher.= $row['idvoucher'].", ";
			if(intval($row['idvoucher']) == intval($idVoucherParam))
			{
				$halaman = $a;
			}
		}
...
function totalPage($CKoneksiVoucher, $yearProcess)
{
	$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher FROM tblvoucher WHERE deletests=0 AND YEAR(datepaid)='".$yearProcess."' GROUP BY batchno ORDER BY 0+batchno DESC;", $CKoneksiVoucher->bukaKoneksi());
...
function detailCari($CKoneksiVoucher, $CVoucher, $CPublic, $idVoucher)
{
	...
	$paidName = $CPublic->konversiQuotes($row['kepada']);
