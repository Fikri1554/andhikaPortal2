...
function getVoyage(idDesc,vslCode,thn)
{
...
if($aksiGet == "tambahDesc" || $aksiGet == "hapusDesc" || $aksiGet == "saveDesc")
{
	...
	if($aksiGet == "saveDesc")
	{
		...
		for($a=0;$a<=($jmlExpAllIdDesc-2);$a++)
		{
			...
			$CHistory->updateLogVoucher($userIdLogin, "Simpan UBAH DESCRIPTION Voucher (iddesc=<b>".$idDescGet."</b>, idvoucher=<b>".$idVoucherGet."</b>, keterangan=<b>".$keteranganGet."</b>, amount=<b>".$descAmountGet."</b>, perkiraan=<b>".$perkiraanGet."</b>, subacc=<b>".$subAccGet."</b>, unit=<b>".$descDivGet."</b>, unitname=<b>".$descDivName."</b>, vescode=<b>".$vesCodeGet."</b>, vesname=<b>".$vesName."</b>, booksts=<b>".$descBookStsGet."</b>, updusrdt=<b>".$userWhoActNew."</b>)");
		}
	}
}
...
//echo $idVoucherGet; 
$query = $CKoneksiVoucher->mysqlQuery("SELECT iddesc, idvoucher, batchno, keterangan, amount, perkiraan, subacc, unit, vescode, booksts, voyage_no FROM tbldesc WHERE idvoucher='".$idVoucherGet."' AND deletests=0 ORDER BY urutan;", $CKoneksiVoucher->bukaKoneksi());
while($row = $CKoneksiVoucher->mysqlFetch($query))
{
	...
	$thnFNya = date("d/m/Y", strtotime($thnAcct));
	...
	$buttonNaik = "<button class=\"".$classBtnHapusDesc."\" onClick=\"ubahUrutanPopUp('naik','".$idDesc."');\" id=\"\" title=\"SEQUENCE UP\" style=\"position:absolute;top:0px;width:35px;height:12px;\" ".$disBtnHapusDesc."><img src=\"../picture/control-090.png\" width=\"15\" style=\"margin-top:-2px;\"/></button>";
	
	$buttonTurun = "<button class=\"".$classBtnHapusDesc."\" onClick=\"ubahUrutanPopUp('turun','".$idDesc."');\" id=\"\" title=\"SEQUENCE DOWN\" style=\"position:absolute;top:58px;width:35px;height:12px;\" ".$disBtnHapusDesc."><img src=\"../picture/control-270.png\" width=\"15\" style=\"margin-top:-2px;\"/></button>";
		...
		<td>
			...
			<tr align="center" valign="middle">
                ...
                <td align="left" style="position: relative;">
                	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;VESSEL&nbsp;&nbsp;&nbsp;
                	<select id="vesCode<?php echo $idDesc; ?>" name="vesCode<?php echo $idDesc; ?>" class="elementMenu" style="width:180px;height:22px;vertical-align:top;" onchange="getVoyage('<?php echo $idDesc; ?>',$(this).val(),'<?php echo $thnFNya; ?>');" <?php echo $disabled; ?>>
						<option value="">&nbsp;</option>
						<?php echo $CVoucher->menuVessel($row['vescode']); ?>
					</select>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;VOYAGE&nbsp;&nbsp;
                	<select id="slcVoyage<?php echo $idDesc; ?>" name="slcVoyage<?php echo $idDesc; ?>" class="elementMenu" style="width:150px;height:22px;vertical-align:top;" <?php echo $disabled; ?>>
						<?php echo $optVoyage; ?>
					</select>
			...
			<tr valign="middle">
            	<td height="35">
                	&nbsp;&nbsp;&nbsp;AMOUNT&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="descAmount<?php echo $idDesc; ?>" name="descAmount<?php echo $idDesc; ?>" value="<?php echo $amount; ?>" class="elementInput" style="width:90px;text-align:right;" onKeyUp="hanyaAngka<?php echo $idDesc; ?>();return false;" <?php echo $disabled; ?>>
                    &nbsp;&nbsp;&nbsp;ACCT #&nbsp;&nbsp;&nbsp;<input type="text" id="perkiraan<?php echo $idDesc; ?>" name="perkiraan<?php echo $idDesc; ?>" value="<?php echo $row['perkiraan']; ?>" class="elementInput" style="width:50px;" maxlength="5" <?php echo $disabled; ?>>
                </td>
                <td align="right">
                	SUB ACCT. <?php echo $i; ?>&nbsp;&nbsp;&nbsp;
                	<input type="text" id="subAcc<?php echo $idDesc; ?>" name="subAcc<?php echo $idDesc; ?>" value="<?php echo $row['subacc']; ?>" class="elementInput" style="width:50px;" maxlength="5" <?php echo $disabled; ?>>
                </td>
                <td>
                	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;DIVISION&nbsp;&nbsp;
                	<select id="descDiv<?php echo $idDesc; ?>" name="descDiv<?php echo $idDesc; ?>" class="elementMenu" style="width:180px;" <?php echo $disabled; ?>>
						<option value="">&nbsp;</option>
...
if($aksiGet == "saveDesc")
{
	...
	$spanAmount = "( ".$CVoucher->detilVoucher($idVoucherGet, "currency")." )&nbsp;&nbsp;".$amountParent;
	$imgWarning = "";
	$warnaAmount = "333";
...
?>
	...
	//$('#amount', parent.document).val('<?php echo $amountParent; ?>aaa');
