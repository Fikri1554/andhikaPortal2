...
$cekUserId = $_SESSION['userIdSession'];
if($cekUserId == "")
{
	echo "<br><br><label style=\"color:red;font-size: 24px;padding-left:50px;\"><b>Session timeout, Please Re-login..!!</b></label>";
	exit;
}
...
<script language="javascript">
	$(document).ready(function(){
		var cekUserId = "<?php echo $userIdSession; ?>";
		if(cekUserId == "")
		{
...
if($aksiGet == "displayVoucher" || $aksiGet == "simpanBaruVoucher" || $aksiGet == "simpanUbahVoucher" || $aksiGet == "closePopUp" || $aksiGet == "deleteVoucher" || $aksiGet == "btnBackVoucher" || $aksiGet == "transToAcct" || $aksiGet == "ketikElementCariVo")
{
	...
	$txtBarcode = $_GET['txtBarcode'];
	$txtDueDate = $CPublic->convTglDB( $_GET['txtDueDate'] );
	
	if($aksiGet == "simpanBaruVoucher")
	{
		//echo $noInvoiceGet." / ".$jobNoGet;
		$cekUserId = $_GET['userId'];
		if($cekUserId != "")
		{
			$userWhoActNew = $CPublic->userWhoActNew($cekUserId);
		}
		// $cekSessionUserId = $_SESSION['userIdSession'];
		// if ($cekSessionUserId != "" AND $cekUserId == "") 
		// {
			// $userWhoActNew = $CPublic->userWhoActNew($cekSessionUserId);
		// }
		$userWhoActNew = $CPublic->userWhoActNew($_SESSION['userIdSession']);
		$CKoneksiVoucher->mysqlQuery("INSERT INTO tblvoucher 
						 (batchno, kepada, voctype, booksts, company, companyname, paytype, bankcode, banksource, voucher, reference, chequeno, invno, jobno, datepaid, currency, additional, barcode, addusrdt,due_date) 
						 VALUES 
	...
	{
		if($txtBarcode != "")// UPDATE tbl mailinvoice di invoice register jika barcode di isi
		{
			$CKoneksiInvReq->bukaKoneksi();
			$datePaidNya = date("Y-m-d");
			$CKoneksiInvReq->mysqlQuery("UPDATE mailinvoice SET paid='Y', datepaid = '".$datePaidNya."' WHERE barcode='".$txtBarcode."' AND deletests=0;");
			$CKoneksiVoucher->bukaKoneksi();
		}

		$sql = " SELECT TOP 1 CASE WHEN '".$datePaidGet."' > datelock THEN 'ya' ELSE 'tidak' END AS stTrans,* FROM lockquarter WHERE Company = '".$companyGet."' ORDER BY datelock DESC ";
		$query = $koneksiOdbcAcc->odbcExec($koneksiOdbcAccId,$sql);
		while($rows = $koneksiOdbcAcc->odbcFetch($query))
		{
			if($rows['stTrans'] == "tidak")
			{
				echo "<p style=\"color:red;\"><i>Check Your Date Paid,</i> Because Date is Lock..!!</p>";
				exit;
			}
		}
	...
	/*		echo $bookDescDesc."<br>";echo $vesselDesc."<br>";echo $accountDesc."<br>";echo $subAccDesc."<br>";echo $subCodeDesc."<br>";echo $bookStsDesc."<br>";echo $amountDesc."<br>";echo "--------------------<br>";*/
			...
			$koneksiOdbcAcc->odbcExec($koneksiOdbcAccId, "INSERT INTO ".$dbAccounting." 
				...
				('".$company."', '".$source."', '".$bookDate."', '".$voucher."', '".$refNumber."', '".$invNo."', '".$batchno."', '".$bookDescDesc."', '".$vesselDesc."', '".$accountDesc."', '".$subAccDesc."','".$subCodeDesc."', '".$currency."', '".$bookStsDesc."', '".number_format((float)$amountDesc, 2, '.', '')."', '".$currency."', '".$codeSts."', '".$entryDate."', '".$payType."', '".$userName."', '".$progDebug."', '".$jobNo."','".$voyageNo."')");	
...
<?php
	//============================= $userCompany di set di configVoucher.php
	$whereCMP = "";
	...
	$CKoneksiVoucher->bukaKoneksi();
	$paramCariGet = $_GET['paramCari'];	
	$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher, batchno, kepada, voctype, company, companyname, paytype, bankcode, voucher, reference, chequeno, invno, jobno, datepaid, currency, amount, additional, trfacct, barcode,file_upload,due_date,buktibayar_file,approve_voucher FROM tblvoucher WHERE (batchno LIKE '%".$paramCariGet."%' OR kepada LIKE '%".$paramCariGet."%' OR companyname LIKE '%".$paramCariGet."%') AND YEAR(datepaid)='".$yearProcessGet."' AND deletests=0 ".$whereCMP." GROUP BY batchno ORDER BY 0+batchno DESC LIMIT ".$offset.",".$limitBatchno.";", $CKoneksiVoucher->bukaKoneksi());
	while($row = $CKoneksiVoucher->mysqlFetch($query))
	{
		$i++;

		$rowColor = $CPublic->rowColorCustom($i, "#FFFFFF", "#F0F1FF");
		$iconTrfAcct = "<img src=\"../picture/hourglass-select-remain.png\" width=\"14\" title=\"WAITING FOR APPROVAL\">";

		if($row['approve_voucher'] == "Y")
		{
			$iconTrfAcct = "<img src=\"../picture/thumb-up.png\" width=\"14\" title=\"APPROVED\">";
		}
		...
		$dueDate = $CPublic->convTglNonDB( $row['due_date'] );
		...
		$batchNo = $CPublic->zerofill($row['batchno'], 6);
		if($row['file_upload'] != "")
		{
			$batchNo = "<a href=\"./fileUpload/".$row['file_upload']."\" target=\"_blank\">".$batchNo."</a>";
		}

		if($row['buktibayar_file'] != "")
		{
			$iconTrfAcct .= "<a href=\"./fileBuktiBayar/".$row['buktibayar_file']."\" target=\"_blank\"><img src=\"../picture/Search2-32.png\" width=\"14\" title=\"FILE BUKTI TRANSFER\"></a>";
		}

		$klikRow = "parent.klikRowVoucher('". $CPublic->zerofill($row['batchno'], 6)."', '".$row['trfacct']."', '".$row['voctype']."', '".$kepada."', '".$row['company']."', '".$row['paytype']."', '".$row['bankcode']."', '".$row['voucher']."', '".$row['reference']."', '".$row['chequeno']."', '".$invNo."', '".$jobNo."', '".$datePaid."', '".$row['currency']."', '".$amount."', '".$additional."', '".$row['barcode']."', '".$dueDate."');";
...
?>
    	<tr align="center" valign="bottom" bgcolor="<?php echo $rowColor; ?>" onMouseOver="this.style.backgroundColor='#D9EDFF';" onmouseout="this.style.backgroundColor='<?php echo $rowColor; ?>';" id="tr<?php echo $row['idvoucher']; ?>" onclick="<?php echo $clikTR; ?>" 
			...
			<td width="55" class="tabelBorderTopLeftNull" style="font-size:11px;color:#096;font-weight:bold;"><?php echo $batchNo;  ?></td>
