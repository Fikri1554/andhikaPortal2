...
//####### Paging #####################################################################################################
	...
	$totalBatchno = totalBatchno($CKoneksiVoucher, $aksiGet, $paramCariGet); // TOTAL KESELURUHAN DATA Batchno
...
//####### END - Paging #####################################################################################################	
	...
	$width = "467";
	$width2 = "230";
	$totalRowVoc = totalRowVoc($CKoneksiVoucher, $aksiGet, $paramCariGet, $offset, $limitBatchno);
	//echo "<br/><br/>".$totalRowVoc."a".$pageBatchno." | ".$totalBatchno." | ".$maxPage;
	if($totalRowVoc >= 20)
	{
		$width = "450"; // selisih 17
		$width2 = "200";
	}
?>
    ...
    <tr align="center">
        <td width="37" class="tabelBorderRightJust" height="30">TRF.<BR>ACCT.</td>
...
?>
    ...
    <tr align="center" valign="bottom" bgcolor="<?php echo $rowColor; ?>" onMouseOver="this.style.backgroundColor='#D9EDFF';" onmouseout="this.style.backgroundColor='<?php echo $rowColor; ?>';" id="tr<?php echo $row['idvoucher']; ?>" onclick="<?php echo $clikTR; ?>" 
            ...
            <td width="37" height="22" class="tabelBorderTopLeftNull"><?php echo $iconTrfAcct; ?></td>
            ...
            <td width="235" class="tabelBorderTopLeftNull" align="left" style="padding-left:2px;"><?php echo $row['kepada']; ?>&nbsp;</td>
...
<?php
function totalBatchno($CKoneksiVoucher, $aksiGet, $paramCariGet)
{
	if($aksiGet == "ketikElementCariVo")// MENCARI HANYA Batchno DI HALAMAN PAYMENT BY BATCH
	{
		$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher FROM tblvoucher WHERE (batchno LIKE '%".$paramCariGet."%' OR companyname LIKE '%".$paramCariGet."%') AND deletests = 0;", $CKoneksiVoucher->bukaKoneksi());
	}
	else
	{
		$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher FROM tblvoucher WHERE deletests=0 GROUP BY batchno ORDER BY 0+batchno DESC;", $CKoneksiVoucher->bukaKoneksi());
	}
...
function totalRowVoc($CKoneksiVoucher, $aksiGet, $paramCariGet, $offset, $limit)
{
	if($aksiGet == "ketikElementCariVo")// MENCARI HANYA Batchno DI HALAMAN PAYMENT BY BATCH
	{
		$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher FROM tblvoucher WHERE (batchno LIKE '%".$paramCariGet."%' OR companyname LIKE '%".$paramCariGet."%') AND deletests = 0 LIMIT ".$offset.", ".$limit.";", $CKoneksiVoucher->bukaKoneksi());
	}
	else
	{
		$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher FROM tblvoucher WHERE deletests = 0 LIMIT ".$offset.", ".$limit.";", $CKoneksiVoucher->bukaKoneksi());
	}
	$jmlRow = $CKoneksiVoucher->mysqlNRows($query);
...
{	
	...
	setTimeout(function()
	{ 
		$('#hrefThickbox',parent.document).attr('href','templates/halPopup.php?aksi=<?php echo $aksi; ?>&idVoucher=<?php echo $idVoucher; ?>&barisTransno=<?php echo $barisTransnoGet; ?>&pageBatchno=<?php echo $pageBatchno; ?>&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=230&width=400&modal=true');
