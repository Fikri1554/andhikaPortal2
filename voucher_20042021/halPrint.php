...
if($aksiGet == "printVoucher")
{
	...
	if(rtrim($CVoucher->detilVoucher($idVoucherGet, "invno")) != "")
		$noInv = "INV.NO.".$CVoucher->detilVoucher($idVoucherGet, "invno");
	
	$tpl->Assign("vocType",  $vocType);
	$tpl->Assign("compName",  $CVoucher->detilVoucher($idVoucherGet, "companyname"));
	$tpl->Assign("batchNo", $CPublic->zerofill($batchNo, 6)); // 2000239711
	...
	while($rowDesc = $CKoneksiVoucher->mysqlFetch($queryDesc))
	{
		...
		/*$amount = number_format((float)$rowDesc['amount'], 2, '.', ',');
		$amountSplit = explode(".",$rowDesc['amount']);
		if($amountSplit[1] == "00") // JIKA DIBELAKANG KOMA ADALAH KOSONG MAKA NILAI DIBELAKANG KOMA TIDAK DITAMPILKAN
		{
			$amount = number_format((float)$rowDesc['amount'], 0, ".", ",");
		}*/
		$amount = number_format((float)$rowDesc['amount'], 2, '.', ',');
		if($amount == 0.00)
		{	
...
/*	$totalAmount = number_format((float)$CVoucher->detilVoucher($idVoucherGet, "amount"), 2, '.', ',');
	$totalAmountSplit = explode(".",$CVoucher->detilVoucher($idVoucherGet, "amount"));
	if($totalAmountSplit[1] == "00") // JIKA DIBELAKANG KOMA ADALAH KOSONG MAKA NILAI DIBELAKANG KOMA TIDAK DITAMPILKAN
	{
		$totalAmount = number_format((float)$CVoucher->detilVoucher($idVoucherGet, "amount"), 0, ".", ",");
	}*/
	$totalAmount = number_format((float)$CVoucher->detilVoucher($idVoucherGet, "amount"), 2, '.', ',');
	if($totalAmount == 0.00)
	{	
	...
	$statusNB = "";
	if(cekStatusNB($CVoucher, $idVoucherGet) == "no")
	{
		$statusNB = " NB";
	}
	
	$additional = $CVoucher->detilVoucher($idVoucherGet, "additional");
	
	$tpl->Assign("additional", $additional);
	$tpl->Assign("printBy", $printBy.$statusNB);
}

function cekStatusNB($CVoucher, $idVoucherGet) // cek status balance
{
	...
	if($jmlAmountCR == $jmlAmountDB) // JIKA BALANCE
	{
		$status = "yes";
	}
	if($jmlAmountCR != $jmlAmountDB) // JIKA TIDAK BALANCE
	{
		$status = "no";
	}
