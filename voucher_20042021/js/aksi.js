...
function pilihBtnTranstoAcct()
{
	...
	{	
		...
		loadIframe('iframeVoucherList', '');
		loadIframe('iframeVoucherList', 'templates/halVoucherList.php?aksi=transToAcct&idVoucher='+idVoucher+'&batchno='+batchno+'&pageBatchno='+page+'&yearProcess='+yearProcess+'&company='+company+'&datePaid='+datePaid); //REFRESH FRAME
	}
	else
	{	
		doneWait();
...
function cekDateLock(company,datePaid)
{
	var stReturn = "";
	$.post('halPostVoucher.php',
	{ aksi : 'checkDateLock',company : company,datePaid : datePaid },
		function(data) 
		{	
			if(data == 'tidak')
			{
