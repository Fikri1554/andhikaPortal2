...
function allElemReadOnlyTrue()
{
	...
	$('#company option[value=XXX]').prop("disabled", false);
	$('#payMethod option').prop("disabled", true);
	$('#payMethod option[value=cheque]').prop("disabled", false);
	$('#bankCode option').prop("disabled", true);
	$('#bankCode option[value='+$('#bankCodeFirst').val()+']').prop("disabled", false);
	...
	$('#currency option[value='+$('#currFirst').val()+']').prop("disabled", false);
...
function pilihBtnSaveDesc()
{
	...
	if(amount != "0") // JIKA NILAI DARI READONLY ELEMENT ADALAH 0 ATAU BELUM DIISI DARI KETIKA SAVE DESRIPTION
	{
		var okCancel = "";
