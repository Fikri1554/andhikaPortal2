...
if($aksiGet == "tambahDesc" || $aksiGet == "hapusDesc" || $aksiGet == "saveDesc")
{
	...
	if($aksiGet == "saveDesc")
	{
		...
		for($a=0;$a<=($jmlExpAllIdDesc-2);$a++)
		{
			...
			$CKoneksiVoucher->mysqlQuery("UPDATE tbldesc SET keterangan='".$keteranganGet."', amount='".$descAmountGet."', perkiraan='".$perkiraanGet."', subacc='".$subAccGet."', unit='".$descDivGet."', unitname='".$descDivName."', vescode='".$vesCodeGet."', vesname='".$vesName."', booksts='".$descBookStsGet."', updusrdt='".$userWhoActNew."' WHERE iddesc='".$idDescGet."' AND deletests=0"); 
