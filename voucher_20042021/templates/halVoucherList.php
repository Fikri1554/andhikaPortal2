...
<script language="javascript">
	$(document).ready(function(){
		var cekUserId = "<?php echo $_SESSION['userIdSession']; ?>";
		if(cekUserId == "")
		{
...
<?php	
	...
	$whereUserNya = $_SESSION['userIdSession'];
	if ($whereUserNya != "")
	{
		$whereUserNya = " AND SUBSTRING_INDEX (addusrdt,'/',1)  = '".$userIdSession."'";
	}
	//===============================
	$paramCariGet = $_GET['paramCari'];	
	$query = $CKoneksiVoucher->mysqlQuery("SELECT idvoucher, batchno, kepada, voctype, company, companyname, paytype, bankcode, voucher, reference, chequeno, invno, jobno, datepaid, currency, amount, additional, trfacct FROM tblvoucher WHERE (batchno LIKE '%".$paramCariGet."%' OR kepada LIKE '%".$paramCariGet."%' OR companyname LIKE '%".$paramCariGet."%') AND YEAR(datepaid)='".$yearProcessGet."' AND deletests=0 ".$whereCMP.$whereUserNya." GROUP BY batchno ORDER BY 0+batchno DESC LIMIT ".$offset.",".$limitBatchno.";", $CKoneksiVoucher->bukaKoneksi());
