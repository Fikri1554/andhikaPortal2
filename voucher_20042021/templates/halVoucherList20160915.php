...
if($aksiGet == "displayVoucher" || $aksiGet == "simpanBaruVoucher" || $aksiGet == "simpanUbahVoucher" || $aksiGet == "closePopUp" || $aksiGet == "deleteVoucher" || $aksiGet == "btnBackVoucher" || $aksiGet == "transToAcct" || $aksiGet == "ketikElementCariVo")
{
	...
	if($aksiGet == "simpanBaruVoucher")
	{
		...
		$CKoneksiVoucher->mysqlQuery("INSERT INTO tblvoucher 
						 (batchno, kepada, voctype, booksts, company, companyname, paytype, bankcode, banksource, voucher, reference, chequeno, invno, jobno, datepaid, currency, addusrdt) 
						 VALUES 
...
<?php	
	...
	while($row = $CKoneksiVoucher->mysqlFetch($query))
	{
		...
		$klikRow = "parent.klikRowVoucher('". $CPublic->zerofill($row['batchno'], 6)."', '".$row['trfacct']."', '".$row['voctype']."', '".$kepada."', '".$row['company']."', '".$row['paytype']."', '".$row['bankcode']."', '".$row['voucher']."', '".$row['reference']."', '".$row['chequeno']."', '".$invNo."', '".$jobNo."', '".$datePaid."', '".$row['currency']."', '".$row['amount']."');";
