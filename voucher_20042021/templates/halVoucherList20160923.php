...
<?php	
	...
	while($row = $CKoneksiVoucher->mysqlFetch($query))
	{
		...
		$klikRow = "parent.klikRowVoucher('". $CPublic->zerofill($row['batchno'], 6)."', '".$row['trfacct']."', '".$row['voctype']."', '".$kepada."', '".$row['company']."', '".$row['paytype']."', '".$row['bankcode']."', '".$row['voucher']."', '".$row['reference']."', '".$row['chequeno']."', '".$invNo."', '".$jobNo."', '".$datePaid."', '".$row['currency']."', '".$amount."');";
